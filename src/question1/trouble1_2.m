%全部亏掉的情况
%一年一年计算，结果比较合理能够跑出完整的结果
clc;clear;close all;
rng(1)
data=readtable("附件2_处理.xlsx","Sheet","2023年统计的相关数据");
data1=readtable("附件2_处理.xlsx","Sheet","2023年的农作物种植情况");
%%
...
%收益减半
shouyi_mean_2=data_get_all(:,1).*price_mean1*0.5-data_get_all(:,2);
...
data_price_result1_mat=data_price_result1_mat';      %正常每亩地收益情况
data_price_result1_mat_2=data_price_result1_mat_2';  %减半每亩地收益情况
...
for j=1:41
    ...
    data_price_result1_mat_mean2(j)=mean(data_price_result1_mat_2(j,index_get_j));
    data_price_result1_mat_chengben1(j)=mean(data_price_result1_mat_chengben(j,index_get_j));
end

%%
%减半的收益
half_money=-sum(max(sum(p.*mianji_zhongzhi_juzheng-data_price_result1_mat1,2),0).*(data_price_result1_mat_mean2'));
% 正常的收益
abnorm_money=sum(min(sum(p.*mianji_zhongzhi_juzheng-data_price_result1_mat1,2),0).*(data_price_result1_mat_mean'));

% 总体收益，都化成负值，最大化问题转化为求最小问题

Objective=abnorm_money+half_money;
...
for j=1:41
    constr=[constr,sum(p(j,:))<=5,sum(p(j,:))>=1];
end
...
for i=1:7
    ...
    writematrix(data_result','result1_2.xlsx','Sheet',num2str(2023+i),'Range','C2:AQ83');
end
% index_get_mat1_result=double(P);
%%
figure
